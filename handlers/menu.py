...
from keybards.default.button import settings_b, study_menu
from utils.db.db_sqlalch import Category, SubCategory, Quiz, Option, session, UserAnswer
from utils.helper.user_helper import show_statistcs, is_admin, all_quizzes
...
@menu_router.message(F.text == '⬅️ O4rtga')
...
async def settings(message: Message):
    await message.answer(text='Quyidagi birini tanlang.', reply_markup=settings_b())
...
async def download_xls(message: Message):
    ...
    for quiz in all_quizzes():
        ...
        text = quiz.text
        a, b, c, d = [opt.text for opt in quiz.options]
